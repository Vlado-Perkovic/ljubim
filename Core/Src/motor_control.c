...
void motor_step(motor_step_t step, float duty_cycle) {
  ...
  switch (step) {
  case MOTOR_STEP_1: // A->B, C is floating
  ...
  case MOTOR_STEP_2: // A->C, B is floating
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET);
  ...
  case MOTOR_STEP_3: // B->C, A is floating
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
    pwm_off(PHASE_A);
  ...
  case MOTOR_STEP_4: // B->A, C is floating
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET);
  ...
  case MOTOR_STEP_5: // C->A, B is floating
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);
    pwm_off(PHASE_B);
  ...
  case MOTOR_STEP_6: // C->B, A is floating
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);
    pwm_off(PHASE_A);
 ...
 * @note This function is configured for a timer with PSC = 127 and a 64MHz clock,
 * resulting in a 2 microsecond tick time (64MHz / (127+1) = 0.5MHz).
...
void set_commutation_period_us_255(uint32_t period_us) {
  ...
  // Each timer tick is 2us, so we need half the number of ticks as the period in us.
  // ARR = (Number of Ticks) - 1
...
void set_commutation_period_us(uint32_t period_us) {
    ...
    // On STM32G0, timers are on the APB1 bus. Their clock is doubled if the APB prescaler is not 1.
    uint32_t pclk1_freq = HAL_RCC_GetPCLK1Freq();
    uint32_t timer_clk_freq = pclk1_freq;
    
    // FIX: Use the correct register definitions for STM32G0 series.
    // It has a single APB prescaler field named 'PPRE' and the DIV1 value is named differently.
    ...
    // Calculate the maximum period this timer can achieve with its current prescaler.
    // max_period_us = (MAX_ARR + 1) * (PSC + 1) * 1,000,000 / timer_clk_freq
    uint64_t max_period_us_64 = (uint64_t)(MAX_ARR + 1) * (prescaler + 1) * 1000000 / timer_clk_freq;
    uint32_t max_period_us = (max_period_us_64 > 0xFFFFFFFF) ? 0xFFFFFFFF : (uint32_t)max_period_us_64;

    // Clamp the requested period to the timer's capabilities and a safe minimum.
    if (period_us > max_period_us) {
        period_us = max_period_us;
    }
    const uint32_t MINIMUM_PERIOD_US = 50; // A practical minimum (e.g., 20kHz commutation)
    if (period_us < MINIMUM_PERIOD_US) {
        period_us = MINIMUM_PERIOD_US;
    }
